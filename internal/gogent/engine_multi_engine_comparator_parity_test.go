...
func TestEngineMultiEngineComparatorParity(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("engine multi+comparator failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("list comparisons failed: %v", err)
	}
	if len(comps) != 1 {
		t.Fatalf("expected exactly 1 comparison stored, got %d", len(comps))
	}
	...
	if !hasOverall {
		t.Fatalf("expected at least one per-config overall_score")
	}
}
