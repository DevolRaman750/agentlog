...
func TestBasicComparator_Compare(t *testing.T) {
    ...
    if cr == nil { t.Fatalf("expected non-nil comparison result") }
    if cr.ExecutionRunID != "run-test" { t.Fatalf("wrong run id") }
    ...
    if _, ok := cr.ConfigurationScores["_summary"]; !ok { t.Fatalf("expected _summary") }
    // Expect cfg2 to win (faster and creative text)
    if cr.BestConfigurationID != "cfg2" { t.Fatalf("expected cfg2 as best, got %s", cr.BestConfigurationID) }
}
