...
func TestResponseTimeScore(t *testing.T) {
    if ResponseTimeScore(0) != 0 { t.Fatalf("zero time => 0") }
    if ResponseTimeScore(500) <= 0 { t.Fatalf("expected positive score") }
    if ResponseTimeScore(1) > 1 { t.Fatalf("capped at 1.0") }
}
...
func TestCreativityScore_IncreasesWithIndicators(t *testing.T) {
    ...
    if boosted <= base { t.Fatalf("expected boosted > base: %f <= %f", boosted, base) }
}
...
func TestFindFastest(t *testing.T) {
    ...
    if fastest == nil || fastest.Response.ResponseTimeMs != 10 { t.Fatalf("expected 10ms fastest") }
}
...
func TestTokenEfficiencyScore(t *testing.T) {
    ...
    if s <= 0 { t.Fatalf("expected positive efficiency score") }
}
...
func TestEstimatedCost(t *testing.T) {
    ...
    if cost <= 0 { t.Fatalf("expected positive cost, got %f", cost) }
}

func TestGetTokenCount(t *testing.T) {
    meta := map[string]interface{}{"a": 10, "b": 20.0, "c": "30", "d": "x"}
    if GetTokenCount(meta, "a") != 10 { t.Fatalf("expected 10") }
    if GetTokenCount(meta, "b") != 20 { t.Fatalf("expected 20") }
    if GetTokenCount(meta, "c") != 30 { t.Fatalf("expected 30") }
    if GetTokenCount(meta, "d") != 0 { t.Fatalf("expected 0 for invalid string") }
}
...
func TestFindMostCreative(t *testing.T) {
    ...
    if FindMostCreative(scores) != "B" { t.Fatalf("expected B as most creative") }
}
