...
--
-- Temporary view structure for view `agent_execution_summary`
--

DROP TABLE IF EXISTS `agent_execution_summary`;
/*!50001 DROP VIEW IF EXISTS `agent_execution_summary`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `agent_execution_summary` AS SELECT 
 1 AS `agent_id`,
 1 AS `first_name`,
 1 AS `last_name`,
 1 AS `team_id`,
...
--
-- Temporary view structure for view `template_function_summary`
--

DROP TABLE IF EXISTS `template_function_summary`;
/*!50001 DROP VIEW IF EXISTS `template_function_summary`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `template_function_summary` AS SELECT 
 1 AS `template_id`,
 1 AS `template_name`,
 1 AS `enable_function_calling`,
 1 AS `total_functions`,
 1 AS `required_functions`,
 1 AS `function_names`,
 1 AS `function_ids`*/;
SET character_set_client = @saved_cs_client;
...
--
-- Final view structure for view `agent_execution_summary`
--
...
/*!50001 VIEW `agent_execution_summary` AS select `a`.`id` AS `agent_id`,`a`.`first_name` AS `first_name`,`a`.`last_name` AS `last_name`,`a`.`team_id` AS `team_id`,`t`.`name` AS `team_name`,`a`.`lifecycle_status` AS `lifecycle_status`,`a`.`max_tokens_per_day` AS `max_tokens_per_day`,`a`.`tokens_used_today` AS `tokens_used_today`,`a`.`last_execution_at` AS `last_execution_at`,`a`.`total_executions` AS `total_executions`,count(`er`.`id`) AS `execution_count`,max(`er`.`created_at`) AS `latest_execution_at`,sum((case when (`er`.`status` = 'completed') then 1 else 0 end)) AS `successful_executions`,sum((case when (`er`.`status` = 'failed') then 1 else 0 end)) AS `failed_executions` from ((`agents` `a` left join `teams` `t` on((`a`.`team_id` = `t`.`id`))) left join `execution_runs` `er` on((`a`.`id` = `er`.`agent_id`))) group by `a`.`id`,`a`.`first_name`,`a`.`last_name`,`a`.`team_id`,`t`.`name`,`a`.`lifecycle_status`,`a`.`max_tokens_per_day`,`a`.`tokens_used_today`,`a`.`last_execution_at`,`a`.`total_executions` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `template_function_summary`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `template_function_summary` AS select `t`.`id` AS `template_id`,`t`.`name` AS `template_name`,`t`.`enable_function_calling` AS `enable_function_calling`,count(`tf`.`function_id`) AS `total_functions`,count((case when (`tf`.`is_required` = true) then 1 end)) AS `required_functions`,group_concat(`f`.`name` order by `tf`.`execution_order` ASC separator ', ') AS `function_names`,group_concat(`f`.`id` order by `tf`.`execution_order` ASC separator ',') AS `function_ids` from ((`execution_templates` `t` left join `execution_template_functions` `tf` on((`t`.`id` = `tf`.`template_id`))) left join `function_definitions` `f` on((`tf`.`function_id` = `f`.`id`))) group by `t`.`id`,`t`.`name`,`t`.`enable_function_calling` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2025-08-04 18:11:53
