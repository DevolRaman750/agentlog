...
echo "🔍 Plausible Analytics Verification for agentlog.scalebase.io"
echo "============================================================="
echo ""

# Test 1: Check if the site is accessible
echo "1. Testing site accessibility..."
if curl -s -o /dev/null -w "%{http_code}" https://agentlog.scalebase.io | grep -q "200"; then
...
if curl -s https://agentlog.scalebase.io | grep -q 'data-domain="agentlog.scalebase.io"'; then
...
SCRIPT_TAG=$(curl -s https://agentlog.scalebase.io | grep -o '<script[^>]*plausible[^>]*>')
if echo "$SCRIPT_TAG" | grep -q 'defer data-domain="agentlog.scalebase.io" src="https://plausible.io/js/script.js"'; then
...
if curl -s https://agentlog.scalebase.io | grep -A 1 -B 1 "Plausible Analytics" | grep -q "script defer"; then
...
CSP_COUNT=$(curl -s https://agentlog.scalebase.io | grep -i "content-security-policy" | wc -l)
...
echo ""
echo "📊 Plausible Analytics should now be working on https://agentlog.scalebase.io"
echo "   - Script is properly embedded in HTML head"
echo "   - Domain is correctly set to 'agentlog.scalebase.io'"
